import { FunctionComponent, createSharedState, UseStateReturn } from "@monster-js/core";
...
export const createStore = <T>(store: T): <K extends keyof T>(context: FunctionComponent, key: K, callback?: (value?: T[K]) => void) => UseStateReturn<T[K]> => {
    ...
    Object.keys(store as any).forEach(key => sharedStates[key] = createSharedState(key, (store as any)[key]));
