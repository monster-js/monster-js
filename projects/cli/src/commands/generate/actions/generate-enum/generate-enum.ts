...
export function generateEnum(name: string, options: ObjectInterface) {
    const baseName = basename(name);
    const config = getConfig();
    const fullDirPath = resolve(process.cwd(), config?.appRootDir || '', name);
    const fullPath = resolve(`${fullDirPath}.enum.ts`)
    ...
    if (fileExists1) {
        logError('Failed to create new enum.');
        return;
    }

    // if not, create the files
    const camelCaseName = kebabToCamelCase(`-${baseName}`);
    const logic = readFile(paths.enum)
        .replace(/__EnumNameCamelCase__/g, camelCaseName);
