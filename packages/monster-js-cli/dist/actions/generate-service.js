...
function generateService(name) {
    ...
    const filenamePascalCase = (0, filename_to_pascal_case_1.filenameToPascalCase)(filename) + 'Service';
    // Generate the target file path by appending the .component.tsx extension
    const targetFilePath = path.join(process.cwd(), wecoConfig.appRoot, `${name}.service.ts`);
...
`;
    ...
    console.log('');
    (0, logger_1.success)(`Service ${filenamePascalCase} created at ${targetFilePath}`);
}
