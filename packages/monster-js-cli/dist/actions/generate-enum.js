...
function generateEnum(name) {
    ...
    const filenamePascalCase = (0, filename_to_pascal_case_1.filenameToPascalCase)(filename) + 'Enum';
    // Generate the target file path by appending the .component.tsx extension
    const targetFilePath = path.join(process.cwd(), monsterConfig.appRoot, `${name}.enum.ts`);
...
`;
    ...
    console.log('');
    (0, logger_1.success)(`Enum ${filenamePascalCase} created at ${targetFilePath}`);
}
